  ...
  <div class="card-body product-info">
    <div class="row">
      ...
      <div class="col-7">
        ...
        @if($product->type === \App\Models\Product\Product::TYPE_CROWDFUNDING)
        <div class="crowdfunding-info">
          ...
          <!-- 如果众筹状态是众筹中，则输出提示语 -->
          @if ($product->crowdfunding->status === \App\Models\Product\Crowdfunding::STATUS_FUNDING)
          <div>此项目必须在
            ...
            @if(Auth::check())
              ...
              @else
                <button class="btn btn-primary disabled">
                  {{ \App\Models\Product\Crowdfunding::$statusMap[$product->crowdfunding->status] }}
                </button>
              @endif
            @else
              <a class="btn btn-primary" href="{{ route('login') }}">请先登录</a>
            @endif
          @elseif($product->type === \App\Models\Product\Product::TYPE_SECKILL)
            @if(Auth::check())
...
@if($product->type == \App\Models\Product\Product::TYPE_SECKILL && $product->seckill->is_before_start)
  ...
  $(document).ready(function () {
    ...
    $('.btn-crowdfunding').click(function () {
      ...
      swal({
        ...
        // 调用众筹商品下单接口
        axios.post('{{ route('order.crowdfunding.store') }}', req)
    ...
    @endif
      ...
      swal({
        ...
        // 调用秒杀商品下单接口
        axios.post('{{ route('order.seckill.store') }}', req)
          .then(function (response) {
            swal('订单提交成功', '', 'success')
              .then(() => {
                location.href = '/orders/' + response.data.id;
